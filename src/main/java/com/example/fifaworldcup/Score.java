...
public class Score extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rv = view.findViewById(R.id.rv_top_scorers);


        DataFromApi dataFromApi = new DataFromApi(container.getContext());
        dataFromApi.getBestScorers(new MainActivity.GetScorers() {
            @Override
            public void getScorers(List<ScorerModel> list) {
                System.out.println(list.get(0));
                rv.setLayoutManager(new LinearLayoutManager(null));
                ScorerAdapter scorerAdapter =  new ScorerAdapter(container.getContext(),list);
                rv.setAdapter(scorerAdapter);
            }
        });
        return view;
    }
