...
public class Standings extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rv = view.findViewById(R.id.rv_phase_poules);
        rv.setLayoutManager(new LinearLayoutManager(null));
        List<GroupeModel> list;
        DataFromApi dataFromApi = new DataFromApi(container.getContext());
        dataFromApi.getStandings(new MainActivity.GetData() {
            @Override
            public void getGroupList(List<GroupeModel> list) {
                PouleAdapter pouleAdapter = new PouleAdapter(container.getContext(),list);
                rv.setAdapter(pouleAdapter);
            }
        });
        return view;
    }
